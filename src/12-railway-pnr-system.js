...
export function processRailwayPNR(pnrData) {
  ...
  const { pnr, train, classBooked, passengers: rawPassengers } = pnrData;
  ...
  let confirmed = 0, waiting = 0, cancelled = 0, rac = 0;
  ...
  const passengers = rawPassengers.map(p => {
    ...
    if (statusLabel === "CONFIRMED") confirmed++;
    else if (statusLabel === "WAITING") waiting++;
    else if (statusLabel === "CANCELLED") cancelled++;
    else if (statusLabel === "RAC") rac++;
  ...
  const allConfirmed = confirmed === totalPassengers;
  const anyWaiting = waiting > 0;

  // Chart is prepared if all non-cancelled passengers are confirmed
  const chartPrepared = (confirmed + waiting + rac) === confirmed;
